...
var cals = {
	initialize: function() {
          ...
          var list = new Array;
          	  list[0] = "<span class='dkInspect_property'><label for='dkInspect_input_height'>Height</label></span><span><input type='text' id='dkInspect_input_height'> px</span>";
              list[1] = "<span class='dkInspect_property'><label for='dkInspect_input_width'>Width</label></span><span><input type='text' id='dkInspect_input_width'> px</span>";
              list[2] = "<span style='float:right'><button id='dkInspect_btn_submit' class='btn'>확인</button><button id='dkInspect_btn_close' class='btn'>닫기</buttton></span>";
	...
	submit: function(){
		var h = $("dkInspect_input_height").value, 
		...
		cals.setDiagonal(h, w, function (cb) {
			...
			var list = new Array;
          	    list[0] = "<span class='dkInspect_property'> height : </span><span>"+ cb.height +" mm (" + h + "px)</span>";
          	    list[1] = "<span class='dkInspect_property'> width : </span><span>"+ cb.width +" mm (" + w + "px)</span>";
          	    list[2] = "<span class='dkInspect_property'> diagonal : </span><span>"+ cb.diagonal +" mm (" + cb.diagonal_px + "px)</span>";
	...
	setDiagonal: function(h, w, callback){
		...
		chrome.storage.local.get("monitors", function(m) {
			 ...
			 var std_res = cb.resolution.split("x");
			 var std_diagonal = Math.sqrt(Math.pow(parseInt(std_res[0]), 2) + Math.pow(parseInt(std_res[1]), 2)).toFixed(2);
			 var std_px = 25.4 / (std_diagonal / cb.monitor); // mm 기준
