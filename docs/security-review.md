...
- **innerHTML 제거 및 텍스트 렌더링 강제:** `src/content/inspector/css-handlers.ts`에서 오버레이 텍스트를 모두 `textContent` 기반으로 렌더링하도록 `setListItemText` 헬퍼를 추가했습니다. 페이지가 제어하는 CSS 값이 그대로 DOM에 삽입되지 않도록 `innerHTML` 경로를 차단하여 교차 사이트 스크립팅 위험을 해소했습니다.
- **안전한 텍스트 설정 일원화:** 공용 `setTextContent` 유틸을 활용해 라벨과 값 설정을 일원화했습니다. 향후 신규 속성 추가 시에도 동일한 안전 경로를 사용하도록 구조를 고정했습니다.
- **정적 분석 방어 추가:** ESLint에 `innerHTML` 접근을 금지하는 규칙을 추가해 새로운 UI 코드에서 동일한 위험이 재도입되는 것을 방지합니다. 계산된 속성(`element['innerHTML']`) 접근까지 차단해 우회 시도를 포함한 애플리케이션 코드 전반을 자동으로 보호합니다.
