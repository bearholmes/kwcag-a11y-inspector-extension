...
import { RGBToHex, RGBToHexStr, getL } from './color-utils.js';
import { getCurrentDocument } from './dom-utils.js';

/**
 * 문자열에서 'px'를 제거하고 반올림한 값을 문자열로 반환
 * @param {string} value - 픽셀 값 문자열 (예: "10.5px")
 * @returns {string} 반올림된 픽셀 값 (예: "11px")
 */
function removeExtraFloat(value) {
 ...
 */
export function hideCSSCategory(category) {
  const document = getCurrentDocument();
 ...
 * @param {HTMLElement} domElement - DOM 요소
 * @param {boolean} hasWidth - 너비 조건
 * @param {boolean} hasHeight - 높이 조건
 * @returns {number} 성공 시 1, 실패 시 0
 */
function setCSSDiagonal(element, opt, domElement, hasWidth, hasHeight) {
  const document = getCurrentDocument();
 ...
 * @param {CSSStyleDeclaration} element - CSS 스타일 객체
 */
export function updateColorBackground(element) {
  setCSSPropertyValue(
    element,
    'color',
 ...
 * @param {HTMLElement} domElement - DOM 요소
 */
export function updateLength(element, opt, domElement) {
  setCSSDiagonal(
    element,
    opt,
    domElement,
 ...
 * @param {CSSStyleDeclaration} element - CSS 스타일 객체
 */
export function updateBox(element) {
  // Height와 Width
  setCSSPropertyIf(
    element,
    'height',
